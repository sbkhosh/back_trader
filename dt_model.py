...
class FractalBollStrat(bt.Strategy):
    params = (
        ("period", 40),
        ("devfactor", 2),
        ("size", 20),
    ...
    def __init__(self):
        self.boll = bt.indicators.BollingerBands(period=self.p.period, devfactor=self.p.devfactor)
        self.fractal = bt.studies.Fractal(self.data, period=5, bardist=0.01) 
    ...
    def next(self):
        ...
        if(not self.position):
            if(self.fractal.fractal_bearish[0] > self.boll.lines.top):
                self.sell(exectype=bt.Order.Stop, price=self.boll.lines.top[0], size=self.p.size)
            if(self.fractal.fractal_bullish[0] < self.boll.lines.bot):
                self.buy(exectype=bt.Order.Stop, price=self.boll.lines.bot[0], size=self.p.size)
        else:
            if(self.position.size > 0):
                self.sell(exectype=bt.Order.Limit, price=self.boll.lines.mid[0], size=self.p.size)
    ...
    def exec_model(data,cash):
        ...
        cerebro.adddata(data)
        cerebro.broker.setcash(cash)
        cerebro.addstrategy(FractalBollStrat)
        print('Starting Portfolio Value: %.2f' % cerebro.broker.getvalue())
        cerebro.run()
        print('Final Portfolio Value: %.2f' % cerebro.broker.getvalue())
        cerebro.plot(style='candlestick')
